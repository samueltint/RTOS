...
This is a program to practice CPU Scheduling algorithm about shortest remaining time first
 ...
 The input data of the cpu scheduling algorithm is (let's do the job in L-5):
...
The Key point is to check which process is the SRTF at the current time unit if a new process jumps in.
...
{
	processes[0].pid = 1;
	processes[0].arrive_t = 0;
	processes[0].burst_t = 8;
	processes[1].pid = 2;
	processes[1].arrive_t = 1;
	processes[1].burst_t = 4;
	processes[2].pid = 3;
	processes[2].arrive_t = 2;
	processes[2].burst_t = 9;
	processes[3].pid = 4;
	processes[3].arrive_t = 3;
